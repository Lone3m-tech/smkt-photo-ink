...
> 将一张有权使用的照片整体重绘为统一的水墨水彩插画，保留构图、主体可辨认特征与来源色彩关系。
...
这不是多风格工具。它以固定处理把画面中的人物、动物、物件与背景一起重绘，保留来源图的长宽比、构图、主体数量、姿势、相对位置与主要色彩关系。
...
- 输入：一张用户提供且有权使用的照片。
- 输出：一张完整重绘的水墨水彩插画，不是局部滤镜或拼贴。
- 重点保留：主体的可辨认特征、服装或动物花纹、场景锚点，以及来源图的构图与主要色彩关系。
...
- 原图关系优先：不任意改变长宽比、构图、主体数量、姿势、相对关系或来源色彩家族。
- 可辨认而非像素复刻：通过轮廓、服装、动物花纹与场景锚点维持识别特征。

## 完整 Demo

以下四组为使用者确认的 Demo。来源照片已获用于该展示的授权，Ink 结果均为 AI 生成示例。它们展示固定处理的预期效果，不保证其他图片、模型或宿主得到相同结果。
